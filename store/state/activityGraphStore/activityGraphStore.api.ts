...
import { DailyActivitySettingsModel, StoreDailyActivities } from './models/models';
...
export const getDailyActivityApi = async (date_to?: string) => {
    console.log(`${BASE_URL}/daily-activity?date_to=${date_to}`);
    return await HttpClient.get(`${BASE_URL}/daily-activity${date_to ? '?date_to=' + date_to : ''}`);
};
...
export const getDailyActivitySettingsApi = async () => {
    return await HttpClient.get(`${BASE_URL}/daily-activity/settings`);
};

export const setDailyActivitySettingsApi = async (model: DailyActivitySettingsModel) => {
    return await HttpClient.post(`${BASE_URL}/daily-activity/settings/update`, model);
};
